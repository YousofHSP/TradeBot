                        ...
                        if (context.Principal?.Identity is ClaimsIdentity claimsIdentity)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(tokenCode))
                            {
                                var token = await apiTokenRepository.Table
                                    .FirstOrDefaultAsync(t => t.Code == tokenCode && t.Status == ApiTokenStatus.Enable);
                                if (token is null)
                                {
                                    context.Response.StatusCode = StatusCodes.Status401Unauthorized;
                                ...
                                if (token.SubSystemId is null &&
                                    ...
                                    !path.Contains("ConfirmNewEmail") &&
                                    !path.Contains("ChangePassword") &&
                                    !path.Contains("GetSubSystems") &&
                                    !path.Contains("SetSubSystem")
                                    )
                                {

                                    context.Response.StatusCode = StatusCodes.Status409Conflict;
                                    context.Fail("زیرسیستم انتخاب نشده است");
                                    return;
                                }
                                ...
                                else
                                {
                                    ...
                                    {
                                        token.Status = ApiTokenStatus.Disable;
