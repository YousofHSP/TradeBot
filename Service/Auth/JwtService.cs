...
namespace Service.Auth
{
        ...
        : IJwtService, IScopedDependency
    {
        ...
        public async Task<AccessToken> GenerateAsync(User user, CancellationToken ct)
        {
            ...
            var subSystems = await SubSystemRepository.TableNoTracking
                .Where(i => i.UserGroups.Any(g => g.Users.Any(u => u.Id == user.Id)))
                .Select(i => i.Id)
                .ToListAsync(ct);
            int? subSystemId = null;
            if (subSystems.Count == 1)
            {
                subSystemId = subSystems.First();
            }
            ...
            var apiTokens = await apiTokenRepository.Table
                .Where(i => i.Status == ApiTokenStatus.Enable && i.Id != apiToken.Id)
