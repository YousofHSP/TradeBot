...
namespace Api.Controllers.v1
{
    ...
    public class CityController : BaseController
    {
        ...
        [HttpGet("[action]")]
        public async Task<ApiResult<List<SelectDto>>> GetSelectList(CancellationToken ct)
        {
            var tokenCode = User.Identity!.FindFirstValue("TokenCode");
            var apiToken = await _apiTokenRepository.TableNoTracking
                .Include(i => i.SubSystem)
                .ThenInclude(i => i.City)
                .FirstOrDefaultAsync(i => i.Code == tokenCode && i.SubSystemId != null);
            if (apiToken is null)
                throw new LogicException("زیر سیستم پیدا نشد");

            var cities = await _repository.TableNoTracking.Where(i => i.ProvinceId == apiToken.SubSystem.City.ProvinceId).ToListAsync(ct);
            return Ok(cities.Select(i => new SelectDto { Id = i.Id, Title = i.Title }).ToList());
